...
def load_dataset(Dataset, split=0.9):
    ...
    if Dataset == 'IN':
        ...
        #mat_data = sio.loadmat(data_path + 'bsin35.mat')
        mat_data = sio.loadmat(data_path + 'bsin30.mat')
        mat_gt = sio.loadmat(data_path + 'Indian_pines_gt.mat')
        #data_hsi = mat_data['indian_pines_corrected']
        data_hsi = mat_data['img_BS']
        gt_hsi = mat_gt['indian_pines_gt']
        K = 3
        TOTAL_SIZE = 10249
    ...
    if Dataset == 'KSC':
        ...
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
    if Dataset == 'HU':
        data_hsi = sio.loadmat(data_path + 'HoustonU.mat')['houstonU']
        # data = sio.loadmat(os.path.join(data_path, 'hu7.mat'))['img_BS']
        gt_hsi = sio.loadmat(data_path + 'HoustonU_gt.mat')['houstonU_gt']
        K = 3
        TOTAL_SIZE = 504856
        VALIDATION_SPLIT =0.95
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
    ...
    data_hsi = PCA(n_components=K, whiten=True).fit_transform(data_hsi)
