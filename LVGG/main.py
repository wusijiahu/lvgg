...
#lr, num_epochs, batch_size = 0.0001, 100, 32
lr, num_epochs, batch_size = 0.001, 5, 128
#lr, num_epochs, batch_size = 0.00050, 100, 32
...
#net = network.HybridSN_network(BAND,CLASSES_NUM).cuda()
#net = network.SSRN_network(BAND, CLASSES_NUM)

for index_iter in range(ITER):
    #net = network.SSRN_network(BAND, CLASSES_NUM)
    net = network.VG(BAND, CLASSES_NUM)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = network.VG(BAND, CLASSES_NUM).to(device)


    #summary(net.to(device), ( img_rows, img_cols, BAND))
    #input_tensor = torch.randn(batch_size, 1, img_cols, img_cols, img_channels)
    #input_tensor = input_tensor.to(device)
    #flops = FlopCountAnalysis(net, input_tensor)
    #print(flop_count_table(flops))
    #net = network.vanillanet_5(in_chans = BAND,num_classes =  CLASSES_NUM)
    #net = network.HybridSN_network(BAND,CLASSES_NUM)
    #optimizer = optim.Adam(net.parameters(), lr=lr)#, weight_decay=0.0001)
    #optimizer = optim.Adam(net.parameters(), lr=lr, weight_decay=1e-6)
    #optimizer = optim.Adam(net.parameters(),lr=0.0010, weight_decay=1e-6)
    ...
    print(f"Peak memory usage: {peak_memory_usage / 1024 ** 2} MiB")
    pred_test_fdssc = []
    tic2 = time.time()
    with torch.no_grad():
